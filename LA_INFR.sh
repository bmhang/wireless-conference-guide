...
BEOF_DIR="/users/bhang/BEOF"

HOST_EC="10.11.16.7"						# EC host IP address
...
#For tiny testbed
#LISTENER_nodes=( 16 27 )
#SNIFFER_nodes=( 16 27 38 )

LISTENER_nodes=( 2 3 4 5 6 8 11 12 13 14 16 17 18 20 22 23 24 25 26 27 31 34 35 36 37 38 40 53 54 55 57 58 60 )
SNIFFER_nodes=( 2 3 4 5 6 8 11 12 13 14 16 17 18 20 22 23 24 25 26 27 31 34 35 36 37 38 40 53 54 55 56 57 58 60 )

#For largest testbed I can get
#LISTENER_nodes=( 2 3 4 6 7 8 10 11 12 13 14 15 16 18 19 20 22 23 24 25 27 28 29 30 35 36 37 38 40 42 53 54 55 57 58 59 )
#SNIFFER_nodes=( 2 3 4 6 7 8 10 11 12 13 14 15 16 18 19 20 22 23 24 25 27 28 29 30 35 36 37 38 40 42 53 54 55 56 57 58 59 )
...
printf "$WIFI_SNIFFER_EXEC  -n $HOST -u $USER -k $PASSWD -d $DB -f int:${GROUP_SNIFFERs[IF]} -s $PKT_STAT"
...
do
	...
	ROHC=0
	codecType=${parameters[18]}
	codecBitRate=${parameters[19]}
	frameLen=${parameters[20]}
	intCOR=${parameters[21]}
#	intPeriod=${parameters[23]}
	intPeriod=4000
#	bgSensing=${parameters[22]}
	...
	exec_WR ${GROUP_SNIFFERs[MCT_ADDR]}  "killall -q wifi_sniffer"
...
#	exec_WR ${GROUP_SNIFFERs[MCT_ADDR]} "killall -q tcpdump"
	...
	printf "$PL_JT_LAT_MOS_PARSER -n $HOST -u $USER -k $PASSWD -d $DB -m $LISTENER_MAC_str -t $MRMT_WINDOW -B WIDE-BAND -p $codecType\n"
	printf "$(eval $PL_JT_LAT_MOS_PARSER -n $HOST -u $USER -k $PASSWD -d $DB -m $LISTENER_MAC_str -t $MRMT_WINDOW -B WIDE-BAND -p $codecType  $REDIR_STDERR_TO_LOGFILE)\n"

	printf "$EI_PARSER -n $HOST -u $USER -k $PASSWD -d $DB -m $SNIFFER_MAC_str -t $MRMT_WINDOW -l $CablelossRx\n"
	printf "$(eval $EI_PARSER -n $HOST -u $USER -k $PASSWD -d $DB -m $SNIFFER_MAC_str -t $MRMT_WINDOW -l $CablelossRx  $REDIR_STDERR_TO_LOGFILE)\n"
	...
	PL_JT_LAT_MOS_PARSER_str=$(eval $PL_JT_LAT_MOS_PARSER -n $HOST -u $USER -k $PASSWD -d $DB -m $LISTENER_MAC_str -t $MRMT_WINDOW -B WIDE-BAND -p $codecType  $REDIR_STDERR_TO_LOGFILE)
