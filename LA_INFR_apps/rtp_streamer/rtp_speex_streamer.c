...
#define MAX_SPEEX_PL_SIZE	(MAX_RTP_PL_SIZE - sizeof(langPkg_t))

#define SPEEX_BITRATE_CNT	11
uint32_t speex_bitrate[SPEEX_BITRATE_CNT] = { 3950, 5750, 7750, 9800, 12800, 16800, 20600, 23800, 27800, 34200, 42200 };
...
uint16_t	frameSize;
uint32_t	bitRate = 16800;		// 16.8kpbs output bitRate
...
int read_WAVE_hdr(frame_t *frame, char *audio_file)
{
	...
	if(memcmp(file_magic_no, WAVE_magic_no1, 4) == 0)
	{
		...
		if(memcmp(file_magic_no, WAVE_magic_no2, 4) == 0)
		{
			...
			frame->cbits = (unsigned char *)calloc(MAX_SPEEX_PL_SIZE, sizeof(char));
		}
	}
	else
	{
...
void rtp_rfc2250_pkt_init(rtp_pkt_t *pkt, uint32_t ssrc)
{
	...
	pkt->b.pt = PT_SPEEX;	// SPEEX payload type
...
// return array index if element exists
int arrayIndex(int value, uint32_t *array, uint8_t size)
{
	uint8_t i;
	for (i=0; i < size; i++)
	{
...
// Search the closest element with in an array
uint32_t closestArrayElement(uint32_t element, uint32_t *array, uint8_t cnt)
{
	uint32_t i, closestElement, closestGap;

	closestElement = array[0];
	closestGap = abs(closestElement - element);

	for(i = 1; i < cnt; i++)
	{
		if(abs(array[i] - element) < closestGap)
		{
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "ha:p:t:f:l:r:P:i:L:")) >= 0)
	{
		switch (c)
		{
			case 'h':
				puts("Description");
				puts("-----------");
				puts(BOLD "rtp_speex_streamer" RESET " is an application used to stream an SPEEX encoded raw WAVE file over a network using RTP encapsulation.\n");
				puts("Argument list");
				puts("-------------");
				puts("-h\t\t\thelp menu");
				puts("-a dstn_addr\t\tDestination IP address where RTP is sent to [eg. -a 235.5.5.5]");
				puts("-p port\t\t\tRTP/RTCP port of the outgoing audio [eg. -p 5004]");
				puts("-t ttl\t\t\tTime to Live value of RTP packets  [eg. -t 1]");
				puts("-f audio_file\t\tPath to audio file or STDIN [eg. -f /path/to/audio/file or -f -]");
				puts("-l frameLen\t\tFrame length in msec (i.e. 20, 40, 60) [eg. -l 20]");
				puts("-r bitRate\t\tEncoder output bitRate in bps. Possible values are [3950, 5750, 7750, 9800, 12800, 16800, 20600, 23800, 27800, 34200, 42200] bps [eg. -r 16800]");
				puts("-P enc_par_path\t\tPath for the shared encoding parameter file [eg. -P /tmp/enc_par]");
				puts("-i RTCP_interval\tInterval of RTP packet to send one RTCP sender report packet [eg. -i 100]");
				puts("-L languages\t\tComma separated language Ids where audio is translated and streamed to (only for simulation) [e.g. -L 1,5]\n");
				puts("Example");
				puts("-------");
				puts(BOLD "./rtp_speex_streamer -a 235.5.5.5 -p 5004 -f sample.wav -P /tmp/enc_par -i 100 -L 1,5\n" RESET);
				puts("Streams an RTP encapsulated audio file (sample.wav) using two languages (i.e. ID 1 & 5) to a multicast address 235.5.5.5 on port 5004.");
				puts("The shared encoding parameter file, to change frameLen and bitRate dynamically, is located at /tmp/enc_par and RTCP sender reports are generated once every 100 RTP packets\n");
				goto exit;
			case 'a':
			...
			case 'L':
				...
				break;
			default:
				fprintf(stderr, "rtp_speex_streamer: missing operand. Type './rtp_speex_streamer -h' for more information.\n");
				goto exit;
		}
	}
	...
	{
		fprintf(stderr, "Either [destination address or audio file] is not specified. Type './rtp_speex_streamer -h' for more information\n");
		goto exit;
	}
	// Check frame length
	if(frameLen != 20 && frameLen != 40 && frameLen != 60)
	{
		fprintf(stderr, "Frame length of %umsec is incorrect. Accepted values are [20, 40 and 60] msec. Frame length is set to 20msec\n", frameLen);
		frameLen = 20;
	}
	// Check if bitRate belongs to the allowed values
	if(arrayIndex(bitRate, speex_bitrate, SPEEX_BITRATE_CNT) == -1)
	{
		fprintf(stderr, "Encoder bitrate %u bps is not allowed. Accepted values are [3950, 5750, 7750, 9800, 12800, 16800, 20600, 23800, 27800, 34200 and 42200] bps.\n", bitRate);
		bitRate = closestArrayElement(bitRate, speex_bitrate, SPEEX_BITRATE_CNT);
		fprintf(stderr, "Encoder bitrate is rounded to %u bps.\n", bitRate);
	}
	...
	if(strcmp(audio_file, "-") == 0)
	{
		...
		audio_frame.cbits = (unsigned char *)calloc(MAX_SPEEX_PL_SIZE, sizeof(char));
	}
	...
	temp = arrayIndex(bitRate, speex_bitrate, SPEEX_BITRATE_CNT);
	speex_encoder_ctl(encoder, SPEEX_SET_QUALITY, &temp);					/* Configures quality (bitrate) in the encoder */
	temp = 0;
	speex_encoder_ctl(encoder, SPEEX_SET_VBR, &temp);					/* Disables variable bitRate (VBR) in the encoder. */
	temp = 10;
	speex_encoder_ctl(encoder, SPEEX_SET_COMPLEXITY, &temp);				/* Highest encoder complexity */
	...
	**/
	while (!finished)
	{
		...
		if(strcmp(enc_par_ts, temp_ts) != 0)
		{
			...
			temp = arrayIndex(bitRate, speex_bitrate, SPEEX_BITRATE_CNT);
			speex_encoder_ctl(encoder, SPEEX_SET_QUALITY, &temp);
		...
		/* Resample the pcm audio data */
		// TODO

		nbBytes = 0;
		uint8_t	frameLen_temp = frameLen;
		do
		{
			int bytes;
			...
			/* Encode the frame */
			speex_encode_int(encoder, audio_frame.in, &bits);

			/* Copy encoded frame */
			bytes = speex_bits_write(&bits, (char *)(audio_frame.cbits + nbBytes + 1), MAX_SPEEX_PL_SIZE - nbBytes);
			if (bytes<0)
			{
				fprintf(stderr, "speex encoding failed\n");
				return EXIT_FAILURE;
			}

			/* Augment size of encoded frame */
			*(audio_frame.cbits + nbBytes) = bytes;
			nbBytes += bytes + 1;

			frameLen_temp -= 20;
		}while(frameLen_temp > 0);
		...
		rtp_pkt.timestamp += audio_frame.frameSize * (16000.0/audio_frame.sampleRate);
		rtp_pkt.seq++;
