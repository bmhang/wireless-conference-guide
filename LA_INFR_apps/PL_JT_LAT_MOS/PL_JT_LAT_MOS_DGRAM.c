...
// For the given interface name, retrieve IP address
void IFtoIP(char *if_name, char *ip_addr)
{
	int fd;
	struct ifreq ifr;
...
// PESQ score calculation out of packet loss, jitter and latency network parameters and mapping into a normalized MOS scale
...
// Insert Packet Loss, Jitter, Latency and MOS_norm score into database
void insert_PL_JT_LAT_MOS_score(MYSQL *conn, uint64_t current_time, char *src_IP, char *sniffer_mac, double averagePacketLoss, double averageJitter, double averageLatency, double MOS_norm)
{
	char query_statement[256];

	// PL_JT_LAT_MOS DB query statement and execution
	snprintf(query_statement, 256, "INSERT INTO PL_JT_LAT_MOS VALUES(%lu,'%s','%s',%f,%f,%f,%f)", current_time, src_IP, sniffer_mac, averagePacketLoss, averageJitter, averageLatency, MOS_norm);
...
int main(int argc, char *argv[])
{
	...
	// BRANDON DEBUG
	printf("PL_JT_LAT_MOS program started.");
	fflush(stdout);


	int option = -1;
	while ((option = getopt (argc, argv, "hn:u:k:d:f:p:P:g:a:i:s:L:")) != -1)
	{
	    switch (option)
	    {
			case 'h':
				...
				fprintf(stderr,"-f iface\t\tinterface to capture traffic [eg. -f wlan0]\n");
				fprintf(stderr,"-p RTPport\t\tRTP port of the incoming audio [eg. -p 5004]\n");
				fprintf(stderr,"-P RTCPport\t\tRTCP port of the incoming audio [eg. -P 5005]\n");
				fprintf(stderr,"-g group\t\tMulticast group [eg. -g 235.5.5.5]\n");
	...
	if(host == NULL || user == NULL || passwd == NULL || db == NULL || iface == NULL || audio_BW == NULL)
	{
		fprintf(stderr,"Either or all of [Host name/User name/Password/Database/Interface/Audio stream BW] is not specified. Type './MOS -h' for more information\n");
		return 1;
	}
	...
	/*
	* Create datagram sockets to receive RTP and RTCP packets.
	*/
	RTPsock = socket(AF_INET, SOCK_DGRAM, 0);
	if (RTPsock < 0)
	{
		fprintf(stderr, "opening RTP datagram socket: %s\n", strerror(errno));
		exit(1);
	}
	RTCPsock = socket(AF_INET, SOCK_DGRAM, 0);
	if (RTCPsock < 0)
	{
		fprintf(stderr, "opening RTCP datagram socket: %s\n", strerror(errno));
		exit(1);
	}
	...
	*/
	if (setsockopt(RTPsock, SOL_SOCKET, SO_REUSEADDR,(char *)&reuse, sizeof(reuse)) < 0)
	{
		fprintf(stderr, "setting RTP SO_REUSEADDR: %s\n", strerror(errno));
		exit(1);
	}
	if (setsockopt(RTCPsock, SOL_SOCKET, SO_REUSEADDR,(char *)&reuse, sizeof(reuse)) < 0)
	{
		fprintf(stderr, "setting RTCP SO_REUSEADDR: %s\n", strerror(errno));
		exit(1);
	}

	/*
	* Bind to RTP/RTCP port and proper address
	*/
	// RTP
	memset(&addr, 0, sizeof(struct sockaddr_in));
	addr.sin_family = AF_INET;
	addr.sin_port = htons(RTPport);
	addr.sin_addr.s_addr  = INADDR_ANY;	// Bind to any address since we expect multicast packets
	if (bind(RTPsock, (struct sockaddr*)&addr, sizeof(struct sockaddr_in)))
	{
		fprintf(stderr, "binding to RTP datagram socket: %s\n", strerror(errno));
		exit(1);
	}
	// RTCP
	memset(&addr, 0, sizeof(struct sockaddr_in));
	...
	addr.sin_addr.s_addr  = INADDR_ANY;	// Bind to any address since we expect multicast packets
	if (bind(RTCPsock, (struct sockaddr*)&addr, sizeof(struct sockaddr_in)))
	{
		fprintf(stderr, "binding to RTCP datagram socket: %s\n", strerror(errno));
		exit(1);
	}
	...
	if(group != NULL)
	{
		...
		IFtoIP(iface, sniffer_IP);

		/*
		* Join the multicast group.
		*/
		addr_group.imr_multiaddr.s_addr = inet_addr(group);
		addr_group.imr_interface.s_addr = inet_addr(sniffer_IP);

		// RTP
		if (setsockopt(RTPsock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&addr_group, sizeof(struct ip_mreq)) < 0)
		{
			fprintf(stderr, "setting RTP IP_ADD_MEMBERSHIP: %s\n", strerror(errno));
			exit(1);
		}
		// RTCP
		if (setsockopt(RTCPsock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&addr_group, sizeof(struct ip_mreq)) < 0)
		{
			fprintf(stderr, "setting RTCP IP_ADD_MEMBERSHIP: %s\n", strerror(errno));
			exit(1);
		}
	}

	// Assign polling file descriptor flags
	pfds[0].fd = RTPsock;
	pfds[1].fd = RTCPsock;
	...
	get_mac_addr(iface, sniffer_mac);
	...
	while (finished == 0)
	{
		...
		if(pfds[0].revents & POLLIN)
		{
			...
			// BRANDON DEBUG
			printf("RTP packet received.");
			fflush(stdout);

			remaining = recvfrom(RTPsock, &recv_message, sizeof(recv_message), 0, (struct sockaddr *)&addr, (socklen_t *)&len);
			if(remaining < 0)
			{
				fprintf(stderr, "receiving RTP message: %s\n", strerror(errno));
				continue;
			}

			// packet timestamp at receiver
			ioctl(RTPsock, SIOCGSTAMP, &systime);
			ts_rx_real = systime.tv_sec + systime.tv_usec/1e6;

			// retrieve the RTP header
			rtp_hdr = (rtp_hdr_t *)recv_message;
			...
			if(RECORD_ptr->RTPcount == -1)
			{
				...
				inet_ntop(AF_INET, &(addr.sin_addr), RECORD_ptr->src_IP, INET_ADDRSTRLEN);
			...
			else
			{
				...
				if((current_time - previous_time) > interval)
				{
					...
					// printf("PL: %f -- JIT: %f -- LAT: %f -- MOS: %f \n", averagePacketLoss, averageJitter, averageLatency, MOS_norm);
					insert_PL_JT_LAT_MOS_score(conn, current_time, RECORD_ptr->src_IP, sniffer_mac, averagePacketLoss, averageJitter, averageLatency, MOS_norm);

//					printf("%f\t%f\t%f\t%f\t%f\t%u\n", averagePacketLoss, averageJitter, averageLatency, bitrate_avg, MOS_norm, ntohs(rtp_hdr->seq));
//					fflush(stdout);
		...
		{
			remaining = recv(RTCPsock, &recv_message, sizeof(recv_message), 0);
			if(remaining < 0)
			{
				fprintf(stderr, "receiving RTCP message: %s\n", strerror(errno));
				continue;
			}

			// packet timestamp at receiver
			ioctl(RTCPsock, SIOCGSTAMP, &systime);
			ts_rx_real = systime.tv_sec + systime.tv_usec/1e6;

			// Gets a pointer to the UDP payload
			payload = (void *)recv_message;
