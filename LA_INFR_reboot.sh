...
HOST_EC="10.11.16.53"						# EC host IP address
...
#	exec_WR ${GROUP_SNIFFERs[MCT_ADDR]} "killall -q tcpdump"
	...
	# ------------------------ Reset for next run of exp ----------------------- #
	exec_WR ${GROUP_LISTENERs[MCT_ADDR]} "killall -q PL_JT_LAT_MOS_DGRAM"
	exec_WR ${GROUP_SNIFFERs[MCT_ADDR]}  "killall -q wifi_sniffer && killall -q dd"

	printf "Rebooting\n"

	exec_WOR ${GROUP_LISTENERs[MCT_ADDR]} "reboot"
	exec_WOR ${GROUP_SPEAKER[MCT_ADDR]} "reboot"

	printf "Sleeping for 2 m\n"
	sleep 2m
...
exec_WR ${GROUP_LISTENERs[MCT_ADDR]} "reboot"
exec_WR ${GROUP_SPEAKER[MCT_ADDR]} "reboot"
